...
# es wird die Einschaltzeit des Pins 6 gemessen.
# Pin 6 = Magnetventil
#import
import os, sys, time
...
debug = False
#locale version
localversion = "1.5 vom 20.02.2019"
#Jobname
Jobname = os.path.basename(sys.argv[0])
#Welcher Pin
GPIOPin = 6
PinLog = "GPIO 6"
PinName = "Magnetventil"
PinFile = "/var/www/html/GPIO6.txt"
MQTTTopic = MQTTmagnetventil
#Setup Pin
...
while True:
	...
	if (Pinstate != Pin):
		...
		if (Pin == 0):
			...
			Popen([executable, '/usr/script/beregnungssensor.py'])
		Pinstate = Pin
	else:
		if (state):
			to_time = datetime.now()
