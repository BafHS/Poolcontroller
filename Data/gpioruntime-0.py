...
# es wird die Einschaltzeit des Pins 0 gemessen.
# Pin 0 = Poolpumpe
#import
import os, sys, time
...
PinLog = "GPIO 0"
PinName = "Pumpe"
PinFile = "/var/www/html/GPIO0.txt"
MQTTTopic = MQTTpoolpumpe
#Setup Pin
...
while True:
	...
	if (Pinstate != Pin):
		...
		if (Pin == 0):
			...
			from_time = datetime.now()
			state = True
			#starte Kontrolle Drucksensor Kanal 6
			Popen([executable, '/usr/script/sandfiltersensor.py'])
			#starte Lese Temperaturen aus aller 20 Sekunden
			Popen([executable, '/usr/script/temperatursensorpumpe.py'])
		Pinstate = Pin
	else:
		if (state):
			to_time = datetime.now()
